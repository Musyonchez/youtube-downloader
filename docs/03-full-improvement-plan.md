...
- [x] **Real WebSocket progress broadcasting.** The previous `/ws` stub was removed as dead code in [02-fixes.md](02-fixes.md). Re-added properly this time: `downloader.py`'s progress hook now reports through a callback, `api/routes.py`'s background download task broadcasts `{type: "progress", video_id, percent}` / `{type: "download_complete", ...}` over the connection manager, and the frontend (`websocket.js`, after the module split below) updates the UI live instead of only polling.
...
- [x] **Split `app.js` into modules.** Was an 800+ line file with everything as global functions/state. Split into `state.js` (shared mutable state, loads first), `api.js` (fetch wrapper + load* functions), `ui.js` (theme/view/modal/toast/keyboard shortcuts), `search.js` (search bar, results rendering, pagination), `queue.js` (queue rendering, add/remove/download, settings save), `websocket.js`, and `main.js` (the `DOMContentLoaded` init, loads last). Kept as plain `<script>` tags sharing global scope rather than ES modules or a bundler -- consistent with "no build step" below. This was a purely mechanical extraction (no logic changes): verified afterward that all 35 function declarations and all 10 top-level variables from the original file appear in the split with none missing, none duplicated, and none extra, and that `app.html`'s new script tags actually serve (200) in the correct load order via a live `TestClient` request. Not verified in an actual browser (no browser tooling available in this environment) -- if anything looks off after a real page load, that's the first thing to check.
